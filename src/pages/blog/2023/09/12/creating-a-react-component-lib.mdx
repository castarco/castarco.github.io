...
description: "Learn how to create a React component library using Rollup, TypeScript and Storybook"
pubDate: 2023/09/12

topic: tech
lang: english
tags: [javascript, typescript, frontend, web-development, react]
# ogImage: [
...
> **NOTE:** Although I've done my best to describe all the relevant details in this article, it might be useful for you as a reader to explore some real [example code](https://github.com/Coder-Spirit/beautiful-tree) in case you find holes in the explanation that follows.
...
- `react` and `react-dom` are necessary for our tests to work
- `rollup` is the module bundler that we'll use. `@rollup/plugin-terser` is a plugin that we'll use to minify our code. `@rollup/plugin-typescript` is a plugin that we'll use to transpile our TypeScript code.
...
Although we can run `pnpm tsc --init` to create a new `tsconfig.json` file, for this example we'll create it manually, so we can explain each option in detail.
...
describe('BeautifulTree', () => {
  it('renders a boring circle', () => {
    ...
    // execution is correct because we saw it in Storybook.
...
```jsonc
{
  ...
  "exports": {
    ...
    ".": {
      // the `types` entry always must be the first one.
      // Unfortunately, this one only applies to CJS files.
